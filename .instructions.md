...
1. **TypeScript**: `cd frontend && npx tsc --noEmit` — must show 0 errors
2. **Build**: `cd frontend && npx vite build` — must succeed
3. **Backend**: Ensure `npm start` in backend runs without crash
4. **Lint**: Check for unused imports and variables
